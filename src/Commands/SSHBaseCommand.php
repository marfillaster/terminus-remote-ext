...
abstract class SSHBaseCommand extends TerminusCommand implements SiteAwareInterface
{
     ...
     * @param array $ssh_options SSH options
     * @return string
     * @throws TerminusProcessException
     */
    protected function executeCommand(array $command_args, array $ssh_options)
    {
        ...
        $ssh_data = $this->sendCommandViaSsh($command_line, $ssh_options);
     ...
     * @param array $ssh_options SSH options
     */
    protected function sendCommandViaSsh($command, array $ssh_options)
    {
        $ssh_command = $this->getConnectionString($ssh_options) . ' ' . ProcessUtils::escapeArgument($command);
     ...
     */
    private function getConnectionString(array $ssh_options)
    {
        $sftp = $this->environment->sftpConnectionInfo();
        return vsprintf(
            'ssh -T %s@%s -p %s -o "StrictHostKeyChecking=no" -o "AddressFamily inet" %s',
            [$sftp['username'], $ssh_options['ssh_host'] ? $ssh_options['ssh_host'] : $sftp['host'], $sftp['port'],
              implode(' ', array_map(function($opt) {
                  return sprintf('-o "%s"', $opt);
              }, $ssh_options['ssh_o']))
            ]
        );
    }
